...
class VoiceEngine(private val context: Context) {
    ...
    // 折叠即时重复：3+ 相同字符 → 1；整串为某子串的 2+ 次重复 → 该子串（仅当子串不超原长一半，避免误删正常长句）。
    private fun dedupRepetition(s: String): String {
        var r = s.replace(Regex("(.)\\1{2,}"), "$1")   // 3+ 同字符 → 1
